...
async def start_v04_command(message: Message) -> None:
    await message.answer(
        ...
        "Новые команды:\n"
        "/pvp_career — карьера и пиковый Elo по сезонам\n"
        "/season_archive — список и таблицы PvP-сезонов\n"
        "/hall_of_fame — чемпионы всех сезонов\n\n"
        "Разбор навыков: /match_review · /pvp_coach\n"
...
/pvp_career вычисляет личную карьеру из собственных сезонных строк и матчей пользователя.
/season_archive и /hall_of_fame показывают те же публичные агрегаты Elo, имена и число
матчей, которые уже используются лидербордами. Архив не создаёт отдельную копию профиля,
не раскрывает стенограммы, аргументы или судейские оценки и не хранит новые данные.
...
и активные Redis-сессии. После удаления исходных строк пользователь исчезает из карьеры,
сезонных архивов и Hall of Fame. Жалобы сохраняются как аудиторские записи, но связь с
удалённым заявителем очищается."""
