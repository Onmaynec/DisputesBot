...
async def start_v04_command(message: Message) -> None:
    await message.answer(
        "⚔️ Добро пожаловать в DisputesBot v0.18!\n\n"
        ...
        "Книги рекордов: /pvp_records · /season_records\n"
        "Итоги сезонов: /season_recap · /compare_seasons · /career_records\n"
...
/match_review, /pvp_coach и /pvp_records доступны только владельцу Telegram user ID и
анализируют только матчи, в которых он участвовал. Личная книга рекордов может показать
лучший собственный судейский счёт, но не раскрывает оценки соперника, тему, аргументы,
стенограмму или вердикт матча. Отчёты вычисляются при запросе и отдельно не сохраняются.

/season_records использует только публичные агрегаты: число побед и матчей, серии,
разницу Elo при апсете и частоту встреч пары. Публичная доска не показывает темы,
match ID, стенограммы, аргументы, вердикты или структурированные судейские оценки.
...
исходных строк пользователь исчезает из карьеры, recap, сравнений, личных и сезонных
книг рекордов, архивов и Hall of Fame. Жалобы сохраняются как аудиторские записи, но
связь с удалённым заявителем очищается."""
