...
async def start_v04_command(message: Message) -> None:
    await message.answer(
        "⚔️ Добро пожаловать в DisputesBot v0.12!\n\n"
        "Тренируйте аргументацию с ботом или участвуйте в PvP-дуэлях.\n\n"
        "Новые команды:\n"
        "/ranked_queue — подбор соперника по Elo\n"
...
для лиги не создаётся, а стенограммы и оценки судьи командами лиг не раскрываются.
...
настройку публичности, персональные вызовы, обе очереди, blocklist, настройки и активные
Redis-сессии. Жалобы сохраняются как аудиторские записи, но связь с удалённым заявителем
очищается."""
