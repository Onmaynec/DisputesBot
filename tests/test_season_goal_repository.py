...
@pytest.mark.asyncio
async def test_reactivating_completed_metric_respects_active_limit() -> None:
    database = Database("sqlite+aiosqlite:///:memory:")
    await database.create_all_for_tests()
    await add_profiles_and_player(database, games=5, wins=0)
    repository = SeasonGoalRepository(database.sessions, max_active_goals=1)

    await repository.set_goal(USER, "season-1", "wins", "1")
    async with database.sessions.begin() as db:
        player = await db.get(PvPPlayerRow, {"user_id": 1, "season": "season-1"})
        assert player is not None
        player.wins = 1
    ...
    await repository.set_goal(USER, "season-1", "matches", "10")
    with pytest.raises(GoalLimitError):
        await repository.set_goal(USER, "season-1", "wins", "2")
    await database.close()
